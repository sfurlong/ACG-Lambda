...
YES_OR_N0_REPROMPTS = [
    ...
    'Be careful adventurer, is your answer yes or no.',
    'You are running out of time adventurer! Please answer yes or no.',
    'Adventurer, is your answer yes or no. If you need a travel tip, say speak to the guide.',
...
class StartAdventureIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        try:
            ...
            if is_user_on_session(handler_input):
                ...
                if is_returning_user(handler_input):
                    ...
                    if has_active_adventure(handler_input):
                        ...
                        speak_output = get_next_question(handler_input.attributes_manager.session_attributes["country"],
                                                         handler_input.attributes_manager.session_attributes[
                                                             "stats_record"], handler_input)
            ...
            speak_output = "Sorry, adventurer! I don't understand what you want to do. That country is probably not supported yet. {}".format(
                VISIT_COUNTRY_REPROMPT)
            reprompt_output = VISIT_COUNTRY_REPROMPT
...
class YesIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        try:
            ...
            speak_output = "Sorry, adventurer! I don't understand what you want to do. {} If so, say visit Italy or Australia.".format(
                VISIT_COUNTRY_REPROMPT)
...
def getRandomFact():
    ...
    fact_record = table.query(
        KeyConditionExpression=Key('RecordNumber').eq(str(record_number)))  # dynamo is case-sensitive
...
def getYesorNoResponse(handler_input, textType):
    ...
    try:
        ...
        question_record = table.query(KeyConditionExpression=Key('CountryId').eq(
            get_country_id(handler_input.attributes_manager.session_attributes["country"])) &
                                                             Key('QuestionNumber').eq(
                                                                 handler_input.attributes_manager.session_attributes[
                                                                     "stats_record"]['Items'][0]['QuestionNumber'] + 1))
        ...
        if question_record['Count'] == 1:
            ...
            speak_output = "<voice name=\"" + get_polly_voice(
                handler_input.attributes_manager.session_attributes["country"]) + "\">" + speak_output + " </voice>"
            ...
            if textType == 'YesResponseText':
                handler_input.attributes_manager.session_attributes["stats_record"]['Items'][0]['MoneyLevel'] += \
                question_record['Items'][0]['YesWealthImpact']
                handler_input.attributes_manager.session_attributes["stats_record"]['Items'][0]['EnergyLevel'] += \
                question_record['Items'][0]['YesEnergyImpact']
            elif textType == 'NoResponseText':
                handler_input.attributes_manager.session_attributes["stats_record"]['Items'][0]['MoneyLevel'] += \
                question_record['Items'][0]['NoWealthImpact']
                handler_input.attributes_manager.session_attributes["stats_record"]['Items'][0]['EnergyLevel'] += \
                question_record['Items'][0]['NoEnergyImpact']

                # determine if the game needs to end; ends if player runs out of health or wealth
            current_wealth = handler_input.attributes_manager.session_attributes["stats_record"]['Items'][0][
                'MoneyLevel']
            current_energy = handler_input.attributes_manager.session_attributes["stats_record"]['Items'][0][
                'EnergyLevel']
            ...
            if is_game_over(handler_input.attributes_manager.session_attributes["stats_record"]):
                speak_output = "<voice name=\"" + get_polly_voice(handler_input.attributes_manager.session_attributes[
                                                                      "country"]) + "\">Oh no adventurer, you don't have enough wealth or energy to continue on your adventure! This means your adventure is over. </voice> "
            ...
            # if they are low on wealth/health -- they need a warning
                ...
                speak_output = "<voice name=\"" + get_polly_voice(handler_input.attributes_manager.session_attributes[
                                                                      "country"]) + "\">Be careful adventurer, you are running low on wealth or energy. If you need a travel tip, say speak to the guide.</voice> "
                speak_output = speak_output + " " + get_next_question(
                    handler_input.attributes_manager.session_attributes["country"],
                    handler_input.attributes_manager.session_attributes["stats_record"], handler_input)
            else:
                speak_output = speak_output + " " + get_next_question(
                    handler_input.attributes_manager.session_attributes["country"],
                    handler_input.attributes_manager.session_attributes["stats_record"], handler_input)
        else:  # record not found
            logger.error("That question number doesn't exist: {}".format(
                handler_input.attributes_manager.session_attributes["stats_record"]['Items'][0]['QuestionNumber']))
        ...
        if currentTurns > maxTurns:
            ...
            table.update_item(
                Key={
                    ...
                    'PlayerNumber': handler_input.attributes_manager.session_attributes["user"]['Items'][0][
                        'PlayerNumber'],
                },
...
def is_returning_user(handler_input):
    logger.info("!!!!system", handler_input.request_envelope.context.system)
    logger.info("!!!!user", handler_input.request_envelope.context.system.user)
    logger.info("!!!!user_id", handler_input.request_envelope.context.system.user.user_id)
    ...
    if stats_record['Count'] == 1:
        if stats_record['Items'][0]['ActiveFlag'] == 'Y':
            if 'country' not in handler_input.attributes_manager.session_attributes:
                handler_input.attributes_manager.session_attributes["country"] = get_country_name(
                    stats_record['Items'][0]['CountryId'])
...
def continue_adventure(handler_input):
    speak_output = "<voice name=\"" + get_polly_voice(handler_input.attributes_manager.session_attributes[
                                                          "country"]) + "\">Welcome back adventurer! It's good to see you!</voice> "

    speak_output = speak_output + get_next_question(handler_input.attributes_manager.session_attributes["country"],
...
def get_next_question(countryname, stats, handler_input):
    ...
    question_record = table.query(KeyConditionExpression=Key('CountryId').eq(get_country_id(countryname)) &
                                                         Key('QuestionNumber').eq(stats['Items'][0][
                                                                                      'QuestionNumber'] + 1))  # current completed question + 1
...
def start_new_adventure(handler_input):
    ...
    handler_input.attributes_manager.session_attributes["country"] = \
    handler_input.request_envelope.request.intent.slots['country'].value
...
def is_game_over(stats):
    ...
    if stats['Items'][0]['MoneyLevel'] <= 0 or stats['Items'][0]['EnergyLevel'] <= 0 or stats['Items'][0][
        'ActiveFlag'] == 'N':
        return True
    else:
        return False
